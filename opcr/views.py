...
def create_user_with_employee(request):
    if request.method == 'POST':
        ...
        if user_form.is_valid() and employee_form.is_valid():
            ...
            employee = employee_form.save(commit=False)
            # employee.user = user  # if you're linking them
            employee.save()

            return redirect('some_success_url')
    else:
        # Don't pass instance of request.user to form
        user_form = UserForm()
        employee_form = EmployeeForm()

        # Force-clearing initial values (as a last resort)
        user_form.fields['username'].initial = ''
