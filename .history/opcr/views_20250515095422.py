...
def opcr_smart_kpi_list(request):
    opcr_smart_kpis = OPCR_Smart_kpi.objects.filter(opcr__division=request.user.employee.division).order_by('-pillar_kpi', '-budget')
...
def opcr_smart_kpi_list_all(request):
    opcr_smart_kpis = OPCR_Smart_kpi.objects.all().order_by('-pillar_kpi', '-name')
