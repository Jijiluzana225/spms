...
def user_profile(request):
    employee = request.user.employee  # Assumes every user has a related Employee
    return render(request, 'opcr/user_profile.html', {'employee': employee})
...
@login_required
def create_user_with_employee(request):
    
    if request.method == 'POST':
        user_form = UserForm(request.POST)
        employee_form = EmployeeForm(request.POST)
        if user_form.is_valid() and employee_form.is_valid():
            # Save user properly (handle password hashing)
            user = user_form.save(commit=False)
            user.set_password(user_form.cleaned_data['password'])
            user.save()

            # Save employee info (assuming you link Employee to user somehow)
            employee = employee_form.save(commit=False)
            # For example, if Employee has a ForeignKey to User:
            # employee.user = user
            employee.save()

            return redirect('some_success_url')  # change as needed
    else:
        # Instantiate blank forms without any instance
        user_form = UserForm()
        employee_form = EmployeeForm()

    return render(request, 'opcr/create_user_employee.html', {
        'user_form': user_form,
        'employee_form': employee_form,
    })
